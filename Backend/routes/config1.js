...
router.post("/users1", (req, res) => {
  ...
  let sql = `Select w_flow,RUMPRequestMEType,(select linkRUMPSpace from linkrumpadminaccess 
      where linkRUMPAdminAccessPK=RUMPRequestApprovalLevel) as ApprovalLevelSpace,
      (select linkRUMPRoleFK from linkrumpadminaccess where linkRUMPAdminAccessPK=RUMPRequestApprovalLevel) as ApprovalLevelRoleId,
      RUMPRequestApprovalLevel,RUMPInitiatorId from datarumprequest inner join linkrumprequestflow on 
      (datarumprequest.RUMPRequestFlowFK = linkrumprequestflow.linkrumprequestflowpk) 
      where RUMPRequestPK =?;`
  con.query(sql, req_id, function (err, result) {
    if (err) {
      console.log(err);
    } else {
      let approvalLevelSpace = result[0].ApprovalLevelSpace;
      let approvalLevelRoleId = result[0].ApprovalLevelRoleId;
      ...
      con.query(`select linkRUMPAdminAccessPK,linkRUMPSpace,linkRUMPRoleFK from linkrumpadminaccess where linkRUMPAdminAccessPK in(?)
                 order by field(linkRUMPAdminAccessPK,?);`,
        [wflowdata, wflowdata], (err, result) => {
          if (err) throw err;
          else {
            if (result.length > 0) {
              let wflowResult = result;
              let ApprovalLevelIndex;
              console.log(wflowResult);
              for (let i = 0; i < wflowResult.length; i++) {
                if (approvalLevelSpace == wflowResult[i].linkRUMPSpace && approvalLevelRoleId == wflowResult[i].linkRUMPRoleFK) {
                  ApprovalLevelIndex = i;
                  break;
                }
              }
