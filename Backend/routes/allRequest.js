...
router.post("/allReq", (req, res) => {
  if (req.body.space != 'undefined') {
    ...
    where linkrumprolefk=0 and linkRUMPSpace=? and RUMPRequestCancelStatus=0 order by RUMPRequestdate desc`, space, (err, result) => {
        if (err) throw err;
    ...
    and RUMPRequestCancelStatus=0 order by RUMPRequestdate desc`, req.body.space, (err, result) => {
        if (err) throw err;
        res.end(JSON.stringify(result))
      })
    } else if (req.body.role == 3 || req.body.role == 4) {
