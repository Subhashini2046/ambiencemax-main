...
router.post("/closedReq",(req,res) =>{
  ...
  if (req.body.role == 0) {
    ...
    where linkRUMPActiveFlag=1 and linkrumprolefk=0 and linkRUMPSpace=? and RUMPRequestStatus='Closed' and RUMPRequestCancelStatus=0 order by rumprequestpk desc `, req.body.space, (err, result) => {
      if (err) throw err;
  ...
  router.post("/complete_reqs",(req,res)=>{
    ...
    if (req.body.role == 0) {
      ...
      RUMPRequestStatus,(RUMPRequestUnreadStatus+0) as UnreadStatus from datarumprequest
      inner join linkrumpadminaccess on RUMPInitiatorId=linkRUMPAdminAccessPK
      where linkrumprolefk=0 and linkRUMPActiveFlag=1 and linkRUMPSpace=? and RUMPRequestStatus='Completed' and RUMPRequestCancelStatus=0 order by RUMPRequestDate desc`, req.body.space, (err, result) => {
        if (err) throw err;
        res.end(JSON.stringify(result))
      })
    } else if (req.body.role == 3 || req.body.role == 4) {
...
//     }
//     con.query(`select RUMPRequestNumber,RUMPRequestPK,RUMPRequestSubject,RUMPRequestType,RUMPRequestDate,
//         RUMPRequestStatus,(RUMPRequestUnreadStatus+0) as UnreadStatus from datarumprequest where rumprequestmetype=? and 
...
//           linkrumpspace=?)) and RUMPRequestStatus='Closed' order by rumprequestpk desc  `,
//       [metype, myrole, req.body.space], (err, result) => {
//         if (err) throw err;
//         res.end(JSON.stringify(result))
//       }
//     )
...
//     con.query(`select RUMPRequestNumber,RUMPRequestPK,RUMPRequestSubject,RUMPRequestType,RUMPRequestDate,
//           RUMPRequestStatus,(RUMPRequestUnreadStatus+0) as UnreadStatus
//            from datarumprequest where rumprequestmetype=? and 
//            rumprequestflowfk in(select linkrumprequestflowpk as wid
//             from linkrumprequestflow where 
//             if(${myrole}=3,REVERSE(SUBSTRING_INDEX(REVERSE(SUBSTRING_INDEX(w_flow, 'c', 1)), ',', 1)),
//             REVERSE(SUBSTRING_INDEX(REVERSE(SUBSTRING_INDEX(w_flow, 'e', 1)), 'r', 1)))=
//             (select linkrumpadminaccesspk as id from linkrumpadminaccess where linkrumprolefk=? and 
//             linkrumpspace=?)) and RUMPRequestStatus='Completed' order by RUMPRequestDate desc`,
//       [metype, myrole, req.body.space], (err, result) => {
//         if (err) throw err;
//         res.end(JSON.stringify(result))
//       }
