...
router.post("/viewStatuss",(req,res)=>{
  ...
  con.query(sql,function(err,result){
    if(err){
      ...
      reqLog=result;
  sql = `Select RUMPRequestStatus,RumprequestLevel,RUMPInitiatorId,w_flow,RUMPRequestMEType from datarumprequest inner join linkrumprequestflow on datarumprequest.RUMPRequestFlowFK = linkrumprequestflow.w_id where RUMPRequestPK = ${reqId};`
  con.query(sql,function(err,result){
    if(err){
      console.log(err);
    }else{
