    ...
    if (err) {
      ...
      res.send(
        JSON.stringify({
          ...
          //                       console.log(w_id);
          //                       sql5 = `select * from requests inner join workflow on(workflow.w_id=requests.w_id) where req_status='Pending' and w_flow like '%${user_id},%' and ${user_id}<>req_level order by req_id desc;select count(*) as "pending" from requests where w_id = '${w_id}' and req_status = 'Pending'`;
